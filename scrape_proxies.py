...
IPPortPatternGlobal = re.compile(rf"{IPPattern.pattern}:(\d{{2,5}})")


def get_headers() -> dict[str, str]:
    """Return default HTTP headers for provider requests."""

    return {"User-Agent": random.choice(USER_AGENTS)}


class Provider:
    """Proxy provider base class."""
    ...
    def __init__(self, url=None, proto=(), max_conn=4, max_tries=3, timeout=20, loop=None):
        ...
        self._proxies: set[tuple[str, str, tuple]] = set()
...
class Proxylist_me(Provider):
    ...
    async def _pipe(self):
        ...
        urls = [f"https://proxylist.me/?page={n}" for n in range(lastId)]
        await self._find_on_pages(urls)
...
class Spys_ru(Provider):
    ...
    async def _pipe(self):
        ...
        data = {"xf0": sessionId, "xpp": 3, "xf1": None}
        method = "POST"
        urls = [
            {"url": url, "data": {"xf1": lvl, **data}, "method": method}
            for lvl in [3, 4]
        ]
        await self._find_on_pages(urls)
...
PROVIDERS = [
    ...
    Provider(url="https://socks-proxy.net/", proto=("SOCKS4", "SOCKS5")),
    Provider(
    ...
    Proxylistplus_com(proto=("HTTP", "CONNECT:80", "HTTPS", "CONNECT:25")),
    ...
    Tools_rosinstrument_com(proto=("HTTP", "CONNECT:80", "HTTPS", "CONNECT:25")),
    Tools_rosinstrument_com_socks(proto=("SOCKS4", "SOCKS5")),
